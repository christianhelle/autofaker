...
class MultipleAnonymousPrimitivesViaDecoratorTests(unittest.TestCase):
    ...
    @autodata(str, int, float, complex, range, bytes, bytearray)
    def test_create_primitives_using_decorator(self, text, number, decimal, complex_type, range_type, buffer, buffer2):
        self.assertIsNotNone(text)
    ...
    @autodata(str, int, float, complex, range, bytes, bytearray, use_fake_data=True)
    def test_create_primitives_using_decorator_with_fakes(self, text, number, decimal, complex_type, range_type, buffer, buffer2):
        self.assertIsNotNone(text)
        self.assertNotEqual(number, 0)
        self.assertNotEqual(decimal, float())
        self.assertNotEqual(complex_type, complex())
